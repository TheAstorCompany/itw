...
<?php include("application/views/admin/common/top_menu.php");?>
        ...
        <script>
	        $(document).ready(function() {
				...
				$("#service_name").autocomplete({
					...
					select: function(e, ui) {
						$.get('<? echo base_url();?>admin/DistributionCenters/getService/'+ ui.item.id, function(data) {
			...
			function setContactsHTML(data) {
				...
				for (a in data.result) {
					...
					$('#rightContainer').append('<h6><a href="JavaScript:void(0)" onclick="deleteContact('+it+', \''+data.result[a].firstName.replace("'", "`")+ ' ' +data.result[a].lastName.replace("'", "`")+'\')">Delete</a> ' + contactInfo + '</h6>');
					it++;     											
				}
			}
			...
			function setServicesHTML(data) {
				...
				for (a in data.result) {
					...
					$('#servicesContainer').append('<h6><a href="JavaScript:void(0)" onclick="deleteService('+it+', \''+data.result[a].title.replace("'", "`")+'\')">Delete</a> ' + data.result[a].title.replace("'", "`") + '</h6>');
        ...
        <div class="content">
        	...
        	<div  id="dialog-confirm" title="Confirmation to delete" style="display:none">
				...
				Do you want to delete '<?php echo $data->name; ?>' ?</p>
			</div>
			<?php endif; ?>			           	
    		<div class="row">
    			<div class="sixteen columns">
    				<a href="<?php echo base_url()?>admin/ManageCompany/DC" class="button">&lt;- Go back</a>
    				<?php if ($data->id) : ?>    				
        	...
        	<div class="row">
				...
				<?php if ($data->id) : ?>        		
          			<h1>Distribution Center</h1><fieldset class="dataentry">
                    <label for="type">Last Updated</label>
          			<?php echo date("m/d/Y h:ia", strtotime($data->lastUpdated));?><br /><br />		 		
          		<?php else:?>
        		
          			<h1>New Distribution Center</h1><fieldset class="dataentry"> 		
          		
          		<?php endif; ?>  
                <?php echo form_open('admin/DistributionCenters/AddEdit/' . $data->id, 'id="form_tab_1"'); ?><!--this is no longer relevant because it's not a tab-->
					<?php echo form_hidden('addupdate',1); ?>
						...
						<input name="phone" type="text" value="<?php echo set_value('phone', $data->phone);?>">
							...
							<input name="fax" type="text" value="<?php echo set_value('fax', $data->fax);?>"><br>

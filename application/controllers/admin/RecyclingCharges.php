...
class RecyclingCharges extends Auth {
	
	public function history() {
		$this->load->view('admin/recycling_charges/recycling_charges_list', $this->assigns);
	}
	...
	public function ajaxList() {
		...
		foreach ($data['data'] as $item) {
			...
			$ajaxData[] = array(
				...
				number_format(floatval($item->total_charge1) + floatval($item->total_charge2) - floatval($item->total_charge3), 2),
				number_format(floatval($item->total_charge3), 2),
	...
	public function __construct() {
		...
		$this->assigns['data']->feeOptions = $this->RecyclingChargesModel->getFeeTypeOptions();
	...
	public function addCharge() {
        ...
        $this->loadChargesAndFeesFromPost();
        ...
        $result['html'] = $this->load->view('admin/recycling_charges/charges_and_fees_ajax.php', $this->assigns, true);
        header('content-type:application/json');
    ...
    private function loadChargesAndFeesFromPost() {
        $this->assigns['charges'] = array();
        $this->assigns['fees'] = array();

        if(isset($_POST['existing_charges'])) {
            foreach($_POST['existing_charges'] as $item) {
                $this->assigns['charges'][] = (object)$item;
            }
            unset($_POST['existing_charges']);
        }

        if(isset($_POST['existing_fees'])) {
            foreach($_POST['existing_fees'] as $item) {
                $this->assigns['fees'][] = (object)$item;
            }
	...
	public function addEdit() {
		...
		$this->load->view('admin/recycling_charges/recycling_charges_add', $this->assigns);
	}

...
<?php include("application/views/admin/common/top_menu.php");?>
    ...
    <div class="content">
        <div class="row remove-bottom" style="margin-bottom:0;">
            <div class="sixteen columns">
                <h1><?php echo ($data->invoiceId > 0 ? 'Construction Invoice #'.$data->invoiceId : 'Enter Construction Invoice');?></h1>
            </div>
            <?php if($data->invoiceId > 0) { ?>
                <div class="row">
                    <div class="sixteen columns">
                        ...
                        if(!$data->is_readonly) {
                            ?>
                            <a href="<?php echo base_url();?>admin/ConstructionInvoice/Delete/<?php echo $data->id;?>" class="button">Delete</a>
                        <?php
                        }
                        ?>
                    </div>
                                ...
                                <div style="width: 490px; vertical-align: top; display: inline-block; float:left;">
                                    <div style="margin-bottom:10px;">
                                        ...
                                        <fieldset class="dataentry">
                                            ...
                                            <label for='budgetNumber'>Budget #</label>
                                            <input id="budgetNumber" name="budgetNumber" type="text" style="width:60px;" value="<?php echo set_value('budgetNumber', $data->budgetNumber);?>" />

                                            <label for='lawsonNumber'>Lawson #</label>
                                            <input id="lawsonNumber" name="lawsonNumber" type="text" style="width:120px;" value="<?php echo set_value('lawsonNumber', $data->lawsonNumber);?>" />
                                    ...
                                    <div style="margin-bottom: 10px; margin-top: 10px;">
                                        ...
                                        <?php
                                            if(!empty($data->id)) {
                                                widget::run('audit_table', 'construction_invoice', $data->id);
                                            }
                                        ?>
                                        <br />
                                        <hr />
                                        ...
                                        if(!$data->is_readonly) {
                                            ...
                                            <div style="text-align: center;">
                                                ...
                                                <button style="background:#13602E; color:#fff" onclick="$('#save_type').val(3); document.getElementById('mainForm').submit();">Save Same Vendor</button>
    ...
    <script type="text/javascript">
        ...
        function populateMaterialCharges() {
            if($('#locationId').val()!='' && $('#vendorId').val()!='') {
                $.post("<?php echo base_url(); ?>admin/ConstructionInvoice/PopulateMaterialCharges", {locationId: $('#locationId').val(), vendorId: $('#vendorId').val()},
                    function(data) {
                        var n = 0;

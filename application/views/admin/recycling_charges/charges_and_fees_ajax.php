				<?php if ((isset($charges) && count($charges) > 0) ||  (isset($fees) && count($fees) > 0 )) {?>				
                	...
                	<ol>
                		...
                		<li charge_k="<?php echo $index; ?>">
                            <?php foreach($charge as $prop=>$val) {
                                echo '<input type="hidden" name="existing_charges['.$index.']['.$prop.']" value="'.$val.'" />'."\n";
                            }
                            ?>
                            <strong><?php
                	...
                	<ol>
                		...
                		<li fee_k="<?php echo $index; ?>">
                            <?php foreach($fee as $prop=>$val) {
                                echo '<input type="hidden" name="existing_fees['.$index.']['.$prop.']" value="'.$val.'" />'."\n";
                            }
                            ?>
                            <strong><?php
